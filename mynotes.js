$(function () {
    ...
    $('#addNote').click(function () {
        $.ajax({
            ...
            success: function (data) {
                if (data.trim() == 'error') {
                    $('#alertContent').html("<div class='alert alert-success alert-dismissible'><a href='#' class='close' data-dismiss='alert' aria-label='close'></a><strong>Error!</strong> There is some problem in inserting a new note.</div>!");
                    //$("#alert").fadeIn();
                } else {
                    //console.log(data);
                    //window.alert(data);
                    ...
                    activeNote = data; 
                    activeNote.trim();// echo of mysqli_insert_id()function.
    ...
    $("textarea").keyup(function () {
        ...
        $.ajax({
            ...
            data: { note: notes ,id: ids},
            success: function (data) {
            console.log(data);
                if (data.trim() == 'error') {
                    $('#alertContent').html("<div class='alert alert-success alert-dismissible'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Error!</strong> There is some problem in updating the data</div>!");
                    $("#alert").fadeIn();
                }
            },
            error: function () {
    ...
    $("#allNotes").click(function () {
        $.ajax({
            ...
            success: function (data) {
                $('#note').html(data);
                showHide(["#addNote", "#edit", "#note"], ["#allNotes", "#notePad"]);
                clickonNote(); clickonDelete();
            },
            error: function () {
                $('#alertContent').text("There was an error with the Ajax Call. Please try again later.");
