...
<?php
session_start();
include('connection.php'); // connection comes from settings.php
...
$sql = "SELECT user_id FROM forgotpassword WHERE `keys` ='$key' AND user_id='$user_id'  AND status='pending'";
...
$sql = "UPDATE forgotpassword SET status='used' WHERE `keys`='$key' AND user_id='$user_id'";
$result = mysqli_query($link, $sql);
if(!$result){
